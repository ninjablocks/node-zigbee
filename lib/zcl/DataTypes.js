...
// TODO: Add field length assertions for all the simple types.
...
module.exports = {
    ...
    switch (dataType) {
      ...
      case 0x0f:
        if (data.length != (dataType-7)) {
          throw new Error(
            'Buffer for field is incorrect length.',
            'Required :', (dataType-7),
            'Actual :', data.length
          );
        }
      ...
      case 0x1f:
        if (data.length != (dataType-23)) {
          throw new Error(
            'Buffer for bitmap field is incorrect length.',
            'Required :', (dataType-23),
            'Actual :', data.length
          );
        }
        builder.buffer(data);
      ...
      case 0x31:
        builder.int32le(data);
        break;

      case 0x42:
        builder.uint8(data.length).string(data);
        break;

      case 0x44:
        builder.uint16le(data.length).string(data);
        break;
  ...
  read: function(parser, dataType, fieldName) {
    ...
    switch (dataType) {
      ...
      case 0x0e:
      case 0x0f:
        parser.buffer(fieldName, (dataType-7));
        break;

      case 0x10:
        parser
          .uint8(fieldName)
          .tap(function() { // Boolean
            this.vars[fieldName] = !!this.vars[fieldName];
          });
        break;

      /*
        TODO: Bitmap. How should we map them?
        (add a function to the buffer that accepts an array of prop names with values?)
      */
      case 0x18:
      case 0x19:
      case 0x1a:
      ...
      case 0x1e:
      case 0x1f:
        parser.buffer(fieldName, (dataType-23));
        break;

      case 0x20:
        parser.uint8(fieldName);
        break;

      case 0x21:
        parser.uint16le(fieldName);
        break;

      case 0x22:
        parser
          .buffer(fieldName+'Buffer', 3)
          .tap(function() {
      ...
      case 0x23:
        parser.uint32le(fieldName);
        break;

      case 0x25:
        parser.uint32le(fieldName+'Part1').uint16le(fieldName+'Part2');
        break;

      case 0x28:
        parser.int8(fieldName);
        break;

      case 0x29:
        parser.int16le(fieldName);
        break;
      ...
      case 0x31:
        parser.int32le(fieldName);
        break;

      case 0x42:
        parser.uint8(fieldName+'Length').string(fieldName, fieldName+'Length');
        break;

      case 0x44:
        parser.uint16le(fieldName+'Length').string(fieldName, fieldName+'Length');
        break;
      ...
      default:
        throw new Error(
          'TODO: zcl/DataTypes.read - Unknown data type ' +
          '0x'+dataType.toString(16) + ' for field ' + fieldName);
    }
