...
/*
ZCLCluster.prototype.makeClusterSpecificCommandFrame = function(manufacturerCode, sequenceNumber, commandIdentifier, payload) {
	return this.makeZCLFrame(
		ZCL.FrameControl.ClusterSpecific,
		manufacturerCode,
		commandIdentifier,
		payload
	);
...
ZCLCluster.prototype.sendClusterSpecificCommand = function(commandId, payload) {
	...
	var frame = this.makeClusterSpecificCommandFrame(null, commandId, payload);
	return this.AFDataRequest(frame);
};
*/
...
ZCLCluster.prototype.discoverAttributes = function(startAttribute, maxAttributes) {
	...
	return this.client.sendZCLFrame(null, this.endpoint.device.shortAddress, this.endpoint.endpointId, this.clusterId, false, null, ZCL.GeneralCommands.DiscoverAttributes, payload)
...
ZCLCluster.prototype._parseReadAttributesResponse = function(data) {
	...
	attributes.forEach(function(a) {
		...
		if (a.status.value === 0) { // We only care if we have it.
			a.name = this.attributes[a.attributeIdentifier]? this.attributes[a.attributeIdentifier].name : 'Unknown attribute. Please add to the XCL XML.';
