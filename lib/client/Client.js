		...
		.tap(function() {
			...
			if (response.status.key == 'ZSuccess') {
				...
				self._getDeviceByShortAddress(response.nwkAddr).then(function(device) {
					debug('_handleDeviceMatchDescriptionResponse', 'Found endpoints', endpointIds, 'for device', device.IEEEAddress);
...
var ZCL_RESPONSE_TIMEOUT = 60 * 1000;
...
Client.prototype.sendZCLFrame = function(params, afParams) {
	...
	if (this._inFlight[sequenceNumber]) {
		deferred.error(new Error('Too many pending requests! Sequence number', sequenceNumber, 'has not had a response, and has not timed out. Consider lowering the timeout.'));
	}
